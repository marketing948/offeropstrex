...
import {
  Trophy, Star, Target, TrendingUp, Zap, Crown, Settings, Award, BarChart3,
} from "lucide-react";

// ─── Icon map
const ICON_MAP: Record<string, React.ElementType> = {
  Target, Star, TrendingUp, Zap, Crown, Trophy, Settings, Award,
};
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-sm">
        ...
        <span className="text-xs text-muted-foreground">{value} / {max} <span className="font-semibold text-foreground">({pct}%)</span></span>
      </div>
...
export default function Goals() {
  ...
  const scores = useMemo(() => computeScores(employees, batches, offers, tasks, cfg), [employees, batches, offers, tasks, cfg]);
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Goals & Scoring</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Track performance, earn ranks, and unlock bonuses</p>
        </div>
        {isAdmin && (
          <Link href="/admin/goals-config">
            <Button variant="outline" size="sm">
              <Settings size={13} className="mr-1.5" /> Goals Engine Settings
            </Button>
          </Link>
        )}
      </div>
              ...
              {myNextRank ? (
                <div className="flex-1 min-w-[180px]">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className={`text-xs font-bold ${(RANK_COLORS[myNextRank.color] ?? RANK_COLORS.slate).text}`}>{myNextRank.name}</span>
                  </div>
                  <div className="h-3 rounded-full bg-muted overflow-hidden">
                    {(() => {
                      const pct = Math.min(100, Math.round(((myScore.total - myRank.minScore) / (myNextRank.minScore - myRank.minScore)) * 100));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        {myScore && (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2"><Target size={15} className="text-muted-foreground" /> KPI Progress</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {kpiTargets.map(kt => {
                ...
                const i = kpiTargets.indexOf(kt);
                return (
                  <ProgressBar key={kt.id} label={kt.name} value={val} max={kt.monthlyTarget}
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2"><Trophy size={15} className="text-muted-foreground" /> Leaderboard</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            <div className="divide-y">
              {scores.slice(0, 8).map((s, i) => {
                ...
                const IconC = ICON_MAP[rank.icon] ?? Target;
                ...
                return (
                  <div key={s.employeeId} className={`flex items-center gap-3 px-4 py-3 ${isMe ? "bg-primary/5" : "hover:bg-muted/30"} transition-colors`}>
                    <span className="text-sm font-bold w-6 text-center text-muted-foreground">{i < 3 ? medals[i] : `${i+1}`}</span>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2"><Zap size={15} className="text-amber-500" /> Combo Bonuses</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2"><Award size={15} className="text-muted-foreground" /> Rank Tiers</CardTitle>
        </CardHeader>
        <CardContent>
                      ...
                      <tr key={s.employeeId} className="hover:bg-muted/20 transition-colors">
                        <td className="py-2.5 px-3">
                          <div className="flex items-center gap-2">
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${col.bg} ${col.text}`}>{s.name.charAt(0)}</div>
